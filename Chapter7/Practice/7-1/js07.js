...
document.getElementById("getFile").onchange = function() {
      ...
      //read file contents of selected file
      let fr = new FileReader();
      fr.readAsText(userFile);

      //Once file is finished loading, display in page
      let sourceDoc = document.getElementById("wc_document");
      fr.onload =  function() {
            sourceDoc.innerHTML = fr.result;

            //Store document text removing html tags
      ...
      function wordCloud(sourceText) {
            //Convert the sourceText to lowercase
            // also removes the leading and trailing whitespace
            sourceText = sourceText.toLowerCase();
            sourceText = sourceText.trim();
            
            //Leave only alphabet characters and whitespaces in text
            ...
            // Place remaining words in new array
            let words = sourceText.split(/\s+/g);

            //Sort in alphabetical order
            words.sort();

            //Create 2D array where each item is an array
            // and contains word + duplicate count 
            let unique = [ [words[0],1] ];

            //Keep an index of unique words
            let uniqueIndex = 0;
